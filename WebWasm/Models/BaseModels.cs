...
public record Company(Guid Id, string LegalName, string LegalAddress, string CorporateEmail, bool EmailVerified, string UNP, CompanyInfo? CompanyInfo, BankAccount? BankAccount, ICollection<Vehicle> Vehicles, ICollection<Producer> Producers);
public record CompanyInfo(Guid Id, Guid RegionId, Location Location, string LegalType, TimeSpan StartWorkingHours, TimeSpan EndWorkingHours, string Photo, ICollection<string> ContactPhones); public record Delivery(Guid Id, uint NetoWeight, double GrossWeight, double EstimatedDistance, TimeSpan EstimatedTime, string State, decimal DeliveryCost, Driver? Driver, ICollection<Document> Documents);
